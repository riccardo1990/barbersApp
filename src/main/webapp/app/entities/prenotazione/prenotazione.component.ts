...
export class PrenotazioneComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.prenotazioneService
            ...
            .subscribe(
                (res: HttpResponse<IPrenotazione[]>) => this.paginatePrenotaziones(res.body, res.headers),
    ...
    registerChangeInPrenotaziones() {
        this.eventSubscriber = this.eventManager.subscribe('prenotazioneListModification', response => this.reset());
    }
    ...
    private paginatePrenotaziones(data: IPrenotazione[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        for (let i = 0; i < data.length; i++) {
            this.prenotaziones.push(data[i]);
        }
    }
